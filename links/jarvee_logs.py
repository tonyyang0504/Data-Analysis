...
class GroupJoiner(LogsProcess):
    ...
    def df_error(self):
        df = self.data
        error = df[df['Status'].str.contains('Group Joiner - Error')].drop_duplicates('Status', ignore_index= True)
    ...
    def df_specified_total(self, urls):
        ...
        filtered_list = []
        for url in urls:
            filtered = df[df['Status'].str.contains('Group Joiner') & df['Status'].str.contains(url)]
            filtered_list.append(filtered)
        df_filtered = pd.concat(filtered_list).drop_duplicates('Status', ignore_index=True)
    ...
    def df_specified_error(self, urls):
        ...
        filtered_list = []
        for url in urls:
            filtered = df[df['Status'].str.contains('Group Joiner - Error') & df['Status'].str.contains(url)]
            filtered_list.append(filtered)
        df_filtered = pd.concat(filtered_list).drop_duplicates('Status', ignore_index=True)
